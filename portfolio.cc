...
Portfolio::Portfolio(QWidget *parent) : QWidget(parent) {
    ...
    QGroupBox *entryGroupBox = new QGroupBox("Current Portfolio", this);
    ...
    connect(addRowButton, &QPushButton::clicked, this, &Portfolio::addEntryRow);
...
void Portfolio::addEntryRow() {
    ...
    connect(amountInput, &QLineEdit::textChanged, this, &Portfolio::updateTotal);
    connect(amountInput, &QLineEdit::editingFinished, this, &Portfolio::updateTotal);
}
...
bool Portfolio::saveEntry(const QString &source, double amount, const QString &month) {
    QSqlQuery query;
    QString user = "admin";
    query.prepare("INSERT INTO portfolio (user, month, source, amount) VALUES (:user, :month, :source, :amount)");
...
void Portfolio::updateFormForMonth() {
    QString selectedMonth = monthSelector->date().toString("MMMM yyyy");
